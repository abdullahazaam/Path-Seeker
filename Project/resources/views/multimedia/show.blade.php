...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 space-y-6">
    ...
    <div class="relative glass-panel rounded-3xl border border-slate-200/80 dark:border-white/10 overflow-hidden shadow-2xl">
        <!-- Header -->
        <div class="relative z-10 bg-gradient-to-r from-rose-950/60 via-purple-950/40 to-slate-950 p-8 border-b border-slate-200/80 dark:border-white/10">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="space-y-2">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider {{ $item->type === 'video' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30' : 'bg-purple-500/20 text-purple-300 border border-purple-500/30' }}">
                    ...
                    <h1 class="text-2xl sm:text-3xl font-black text-white leading-snug font-display">{{ $item->title }}</h1>
                </div>
                <span class="text-xs text-slate-400 font-mono shrink-0">Duration: {{ $item->duration ?? '25:00' }}</span>
            </div>
        </div>
        ...
        <div class="relative z-10 p-6 sm:p-8 space-y-6">
            <div class="rounded-2xl overflow-hidden border border-slate-200/80 dark:border-white/10 bg-slate-950 shadow-2xl relative">
                ...
                @else
                    <div class="py-14 px-8 text-center space-y-6">
                        ...
                        <div>
                            <h3 class="text-lg font-black text-white">Audio Stream / Podcast</h3>
                            <p class="text-xs text-slate-400 mt-1">High-definition career podcast</p>
                        </div>
            ...
            <!-- Description -->
            @if($item->description)
                <div class="p-5 rounded-2xl bg-slate-100/70 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] space-y-2">
            ...
            <!-- Tags -->
            <div class="space-y-2">
                <div class="text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-400 flex items-center gap-1.5">
                    <i class="fa-solid fa-tags text-indigo-600 dark:text-indigo-400 text-xs"></i>
                    <span>Topic Tags</span>
                </div>
                <div class="flex flex-wrap gap-2">
                    @foreach(explode(',', $item->tags) as $tag)
                        <span class="px-3 py-1.5 text-xs font-semibold glass-panel rounded-xl border border-slate-200/80 dark:border-white/10 text-slate-800 dark:text-slate-300 flex items-center gap-1.5">
            ...
            <div class="pt-4 border-t border-slate-200/80 dark:border-white/10 flex flex-wrap gap-3">
                <a href="{{ route('multimedia.index') }}" class="px-5 py-2.5 glass-panel rounded-xl text-sm font-bold text-slate-800 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white border border-slate-200/80 dark:border-white/10 hover:border-purple-500/30 transition-all flex items-center gap-2">
                    <i class="fa-solid fa-arrow-left text-xs"></i>
                    <span>Browse All Media</span>
                </a>
                <a href="{{ route('careers.index') }}" class="px-5 py-2.5 rounded-xl font-black text-sm text-white bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 shadow-neon-purple hover:shadow-neon-pink transition-all flex items-center gap-2">
