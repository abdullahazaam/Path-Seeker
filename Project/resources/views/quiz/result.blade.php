...
<div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-8 md:space-y-10 my-6 md:my-8">
                        ...
                        @foreach($domainCounts as $domain => $pct)
                            <div class="p-3.5 rounded-2xl bg-slate-100/80 dark:bg-slate-950/40 border border-slate-200/80 dark:border-white/5 space-y-1.5">
                                ...
                                <div class="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                    <div class="h-full bg-gradient-to-r from-cyan-400 via-sky-500 to-blue-600 rounded-full" style="width: {{ $pct }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="bg-white dark:bg-[#080B12] border border-slate-200 dark:border-white/10 rounded-3xl p-6 sm:p-7 flex flex-col justify-between group relative overflow-hidden shadow-xl dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] hover:-translate-y-1 hover:shadow-2xl transition-all duration-300 hover:border-cyan-500/40">
                    <div class="space-y-4">
                        <div class="flex items-start justify-between gap-2 flex-wrap">
                            <span class="text-[11px] font-black uppercase px-3 py-1 rounded-full bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 border border-cyan-500/20 font-mono shadow-sm">
                        ...
                        <div>
                            <h3 class="text-lg sm:text-xl font-black text-slate-900 dark:text-white group-hover:text-cyan-400 transition-colors font-display">
                                {{ $career['title'] }}
                            </h3>
                        ...
                        @if(!empty($career['reason']))
                            <div class="p-3.5 rounded-2xl bg-cyan-500/10 dark:bg-cyan-500/10 border border-cyan-500/20 text-xs text-cyan-950 dark:text-cyan-200 leading-relaxed">
                                <strong class="font-bold flex items-center gap-1.5 text-cyan-700 dark:text-cyan-300 mb-1">
                        ...
                        @if(!empty($career['required_skills']))
                            <div class="p-3 rounded-xl bg-slate-100/80 dark:bg-slate-950/50 border border-slate-200/80 dark:border-white/10 text-xs text-slate-700 dark:text-slate-300 font-medium">
                                <strong class="text-slate-900 dark:text-white font-bold"><i class="fa-solid fa-code text-cyan-500 mr-1.5"></i>Target Skills:</strong> {{ $career['required_skills'] }}
                            </div>
                        @endif
                    </div>

                    <div class="mt-5 pt-4 border-t border-slate-200/80 dark:border-white/10 flex items-center justify-between">
                        <a href="{{ route('careers.show', $career['career_id'] ?? $career['id']) }}" class="inline-flex items-center gap-2 text-sm font-bold text-cyan-600 dark:text-cyan-400 hover:text-sky-300 transition-colors group/link">
                ...
                @foreach($details as $idx => $d)
                    <div class="p-4 rounded-2xl bg-slate-100/80 dark:bg-slate-950/40 border border-slate-200/80 dark:border-white/10 transition-all space-y-2">
                        ...
                        <div class="text-xs flex flex-wrap items-center justify-between gap-2 text-slate-600 dark:text-slate-400">
                            ...
                            <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-700 dark:text-cyan-300 border border-cyan-500/20 font-mono text-[10px] font-bold">
    ...
    <div class="flex flex-wrap justify-center gap-4 pt-2">
        ...
        <a href="{{ route('dashboard') }}" class="btn-sweep px-8 py-3.5 rounded-full font-black text-xs sm:text-sm text-slate-950 bg-gradient-to-r from-[#00f2fe] via-sky-400 to-blue-600 hover:from-cyan-300 hover:to-blue-500 shadow-[0_0_25px_rgba(0,242,254,0.35)] hover:shadow-[0_0_35px_rgba(0,242,254,0.55)] transition-all flex items-center gap-2 hover:scale-105 cursor-pointer">
            <i class="fa-solid fa-gauge-high text-xs text-slate-950"></i>
            <span class="text-slate-950 font-black">Back to Dashboard</span>
