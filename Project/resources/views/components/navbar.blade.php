...
<header x-data="{ mobileMenuOpen: false }" class="fixed top-5 left-1/2 -translate-x-1/2 w-[96%] max-w-[88rem] z-50">
    <nav class="max-w-[88rem] mx-auto py-4 md:py-4.5 px-6 sm:px-8 md:px-12 bg-white/30 dark:bg-slate-950/80 backdrop-blur-2xl border border-slate-200/50 dark:border-white/10 rounded-full flex items-center justify-between gap-4 md:gap-6 shadow-lg dark:shadow-2xl relative overflow-hidden transition-all duration-300">
        ...
        <!-- Logo & Brand Emblem -->
        <a href="{{ route('home') }}" class="flex items-center gap-2.5 sm:gap-3 group shrink-0 relative z-10">
            <div class="relative">
                <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-600 flex items-center justify-center text-white font-black text-base shadow-neon-purple group-hover:scale-110 group-hover:shadow-neon-pink transition-all duration-300">
                    <i class="fa-solid fa-compass text-white text-base"></i>
                </div>
            ...
            <div class="hidden sm:flex items-center gap-2">
                <span class="text-xl font-black tracking-tight text-slate-900 dark:text-white font-display">PathSeeker</span>
        ...
        <div class="hidden md:flex items-center gap-1 lg:gap-1.5 px-3.5 py-1.5 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-full border border-slate-200 dark:border-white/10 shrink-0 relative z-10 shadow-sm">
            <a href="{{ route('home') }}" class="relative px-3.5 lg:px-4 py-1.5 rounded-full text-xs lg:text-sm font-semibold transition-all {{ request()->routeIs('home') ? 'text-indigo-600 dark:text-indigo-300 font-bold bg-indigo-500/15 border border-indigo-500/30 shadow-sm' : 'text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white hover:bg-white/80 dark:hover:bg-slate-800/80' }}">
                <span>Home</span>
            </a>
        ...
        <div class="flex items-center gap-2.5 sm:gap-3 shrink-0 relative z-10">
            ...
            @auth
                ...
                @endphp
                <div class="hidden sm:flex items-center gap-2.5 px-3.5 py-2 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-full border border-slate-200 dark:border-white/10 shrink-0 shadow-sm">
                    ...
                    <span class="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">{{ $firstName }}</span>
                    <span class="hidden md:inline-block text-[9px] font-black uppercase px-2 py-0.5 bg-purple-500/15 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 rounded-full border border-purple-500/25 dark:border-purple-500/30 shrink-0">{{ Auth::user()->role }}</span>
                </div>

                <a href="{{ route('dashboard') }}" class="btn-sweep inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 text-xs sm:text-sm font-bold rounded-full text-white bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 shadow-neon-purple hover:shadow-neon-pink transition-all duration-300 hover:-translate-y-0.5 shrink-0 whitespace-nowrap">
                ...
                <form action="{{ route('logout') }}" method="POST" class="inline shrink-0 m-0 p-0">
                    ...
                    <button type="submit"
                        title="Sign Out"
                        class="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-white/80 dark:hover:bg-slate-800/80 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-500/40 transition-all shadow-sm shrink-0 cursor-pointer group">
            ...
            @else
                <a href="{{ route('login') }}" class="px-4 py-2 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white transition-colors hidden sm:inline-flex items-center gap-1.5 shrink-0">
                    <span>Sign In</span>
                </a>
                <a href="{{ route('register') }}" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-medium px-6 py-2.5 rounded-full shadow-lg shadow-purple-500/25 transition-all duration-300 inline-flex items-center gap-2 text-xs sm:text-sm shrink-0 whitespace-nowrap">
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                    ...
                    class="md:hidden w-10 h-10 rounded-full bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-white/80 dark:hover:bg-slate-800/80 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm shrink-0 cursor-pointer">
